...
This is a "fork" of https://github.com/zeit/next.js/tree/canary/examples/with-cookie-auth using the `next-authentication` library instead.
...
This example uses `next-authentication` for handling user session in the front-end.
...
Session is synchronized across tabs. If you logout your session gets removed on all the windows as well. We use the HOC `withAuth` for this.
